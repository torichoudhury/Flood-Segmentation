...
# ✅ Load Dataset
data_path = "D:/Flood Segmentation/v1.1/data/flood_events/HandLabeled"
train_dataset = FloodDataset(data_path)
train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=2, pin_memory=True, prefetch_factor=2)
...
for epoch in range(start_epoch, args.epochs):
    ...
    for step, (images, masks) in enumerate(progress_bar):
        ...
        with torch.cuda.amp.autocast():
            outputs = model(images, images)  # Passing SAR twice (optical not used)
        ...
        if (step + 1) % args.accumulation_steps == 0 or step == len(train_loader) - 1:
