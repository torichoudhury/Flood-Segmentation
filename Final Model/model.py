...
# Complete Flood Segmentation Model
class FloodSegmentationModel(nn.Module):
    def __init__(self, use_aspp_cbam=True):
        super(FloodSegmentationModel, self).__init__()
        
        # Feature Extraction
        self.feature_extractor = FloodFeatureExtractorWithTransformer()
        
        # Optional ASPP + CBAM for feature enhancement
        self.use_aspp_cbam = use_aspp_cbam
        if use_aspp_cbam:
            self.aspp_cbam = ASPP_CBAM(in_channels=512, out_channels=512)
    ...
    def forward(self, sar, optical):
        ...
        # Apply ASPP + CBAM if enabled
        if self.use_aspp_cbam:
            features = self.aspp_cbam(features)
...
if __name__ == "__main__":
    ...
    # Forward pass
    segmentation_mask = model(sar_input, optical_input)
    print("Final Output Shape:", segmentation_mask.shape)  # Expected: [4, 1, 64, 64]